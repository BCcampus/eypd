importScripts(`${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/transformstream.js`), importScripts(`${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/idb.js`), importScripts(`${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/pubsubhub.js`), importScripts(`${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/bg-sync-manager.js`), importScripts(`${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/analytics-sw.js`);
const VERSION = '0.0.3407';
self.oninstall = (a) => {
    a.waitUntil(async function () {
        const a = await caches.open('pwp');
        return await a.addAll([`${_wordpressConfig.templateUrl}/header.php?fragment=true`, `${_wordpressConfig.templateUrl}/?fragment=true`, `${_wordpressConfig.templateUrl}/footer.php?fragment=true`, `${_wordpressConfig.templateUrl}/lazy.css`, `${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/import-polyfill.js`, `${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/analytics.js`, `${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/ric-polyfill.js`, `${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/pubsubhub.js`, `${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/router.js`, `${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/pwp-view.js`, `${_wordpressConfig.templateUrl}/dist/scripts/pwa/pwa/pwp-spinner.js`, `${_wordpressConfig.templateUrl}/components/fonts.css`, `${_wordpressConfig.templateUrl}/fonts/Catamaran-Black.woff`, `${_wordpressConfig.templateUrl}/fonts/Catamaran-Bold.woff`, `${_wordpressConfig.templateUrl}/fonts/Catamaran-Medium.woff`, `${_wordpressConfig.templateUrl}/fonts/Catamaran-Light.woff`].map((a) => new Request(a, {credentials: 'include'}))), await Promise.all(['https://www.google-analytics.com/analytics.js'].map((a) => new Request(a, {mode: 'no-cors'})).map(async (b) => await a.put(b, (await fetch(b))))), self.skipWaiting()
    }())
}, self.onactivate = (a) => {

...
function em_maps_load_locations(el){
	...
	// [URL]?width=100%&height=450px&em_ajax=true&query=GlobalMapData&random_id=10d6e
			...
			//define default options and allow option for extension via event triggers
			  ...
			  for (var i = 0; i < data.length; i++) {
				  if( !(data[i].location_latitude == 0 && data[i].location_longitude == 0) ){
					...
					var myContent = '<div class="em-map-balloon ppp"><div id="em-map-balloon-'+map_id+'" class="em-map-balloon-content">'+ data[i].location_balloon +'</div></div>';
			...
			if(jQuery('#em-map').length > 0){
				jQuery.getJSON(document.URL,{ em_ajax_action:'get_location', id:id }, function(data){
					if( data.location_latitude!=0 && data.location_longitude!=0 ){
						...
						map.panBy(40,-55);
						infoWindow.setContent( '<div id="location-balloon-content" class="eypd">'+ data.location_balloon +'</div>');
						infoWindow.open(map, marker);
						google.maps.event.trigger(map, 'resize');
						jQuery(document).triggerHandler('em_maps_location_hook', [map, infowindow, marker, 0]);
					}else{

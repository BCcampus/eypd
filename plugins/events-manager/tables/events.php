 ...
 */
?>
<p class="margin-up"><i>Click on the links below to begin editing an event
		post.</i></p>
<?php
...
if ( ! empty( $show_add_new ) && current_user_can( 'edit_events' ) ) {
	echo '<a class="em-button button add-new-h2" href="' . em_add_get_params( $_SERVER['REQUEST_URI'], [
			'action'   => 'edit',
			'scope'    => null,
			'status'   => null,
			'event_id' => null,
...
<div class="wrap">
	...
	<form id="posts-filter" action="" method="get">
		<div class="subsubsub">
			...
			<a href='<?php echo em_add_get_params( $_SERVER['REQUEST_URI'], $default_params + [ 'view' => 'future' ] ); ?>' <?php echo ( ! isset( $_GET['view'] ) ) ? 'class="current"' : ''; ?>><?php _e( 'Upcoming', 'events-manager' ); ?>
				<span class="count">(<?php echo $future_count; ?>)</span></a>
			&nbsp;|&nbsp;
			<?php if ( $pending_count > 0 ) : ?>
				<a href='<?php echo em_add_get_params( $_SERVER['REQUEST_URI'], $default_params + [ 'view' => 'pending' ] ); ?>' <?php echo ( isset( $_GET['view'] ) && $_GET['view'] === 'pending' ) ? 'class="current"' : ''; ?>><?php _e( 'Pending', 'events-manager' ); ?>
					<span class="count">(<?php echo $pending_count; ?>
						)</span></a> &nbsp;|&nbsp;
			<?php endif; ?>
			<?php if ( $draft_count > 0 ) : ?>
				<a href='<?php echo em_add_get_params( $_SERVER['REQUEST_URI'], $default_params + [ 'view' => 'draft' ] ); ?>' <?php echo ( isset( $_GET['view'] ) && $_GET['view'] === 'draft' ) ? 'class="current"' : ''; ?>><?php _e( 'Draft', 'events-manager' ); ?>
					<span class="count">(<?php echo $draft_count; ?>
						)</span></a> &nbsp;|&nbsp;
			<?php endif; ?>
			<a href='<?php echo em_add_get_params( $_SERVER['REQUEST_URI'], $default_params + [ 'view' => 'past' ] ); ?>' <?php echo ( isset( $_GET['view'] ) && $_GET['view'] === 'past' ) ? 'class="current"' : ''; ?>><?php _e( 'Past Events', 'events-manager' ); ?>
